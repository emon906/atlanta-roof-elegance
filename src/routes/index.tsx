...
function Index() {
  ...
  return (
    <main id="top" className="bg-background text-foreground">
      ...
      <section className="relative flex min-h-[70svh] items-center justify-center overflow-hidden"><video autoPlay muted loop playsInline className="absolute inset-0 h-full w-full object-cover"><source src={heroVideo.url} type="video/mp4" /></video><div className="absolute inset-0 bg-background/60"/><div className="relative px-5 text-center"><Button variant="glass" size="icon" aria-label={playing ? "Pause film" : "Play film"} onClick={() => setPlaying(!playing)} className="mx-auto h-20 w-20 rounded-full transition-transform hover:scale-110"><Play className="ml-1 fill-current" /></Button><p className="mt-7 text-xs font-bold uppercase tracking-[0.25em] text-primary">The Perimeter standard</p><h2 className="mx-auto mt-4 max-w-3xl text-4xl font-semibold md:text-6xl">Precision you can see. Protection you can trust.</h2></div></section>
